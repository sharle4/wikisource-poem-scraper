...
class ScraperOrchestrator:
    ...
    def __init__(self, config):
        ...
        if self.config.tree_log:
            ...
            logger.info(
                f"Hierarchical logging enabled. Logs will be saved to {log_dir}"
            )
    ...
    async def run(self):
        ...
        async with WikiAPIClient(self.api_endpoint, self.config.workers) as client:
            pbar = tqdm(
                desc="Processing pages", unit=" page", dynamic_ncols=True, position=0
            )

            producer_task = asyncio.create_task(
                self._producer(client, page_queue, pbar)
            )
    ...
    async def _producer(self, client: WikiAPIClient, queue: asyncio.Queue, pbar: tqdm):
        ...
        if not page_info or not page_info.get("pages") or "missing" in page_info["pages"][0]:
            logger.warning(
                f"Category '{full_cat_title}' not found. Attempting search..."
            )
            corrected_title = await client.search_for_page(full_cat_title, namespace=14)
            if not corrected_title:
                logger.critical(
                    f"Root category '{self.config.category}' not found. Aborting."
                )
                return
        ...
        logger.info(
            f"Phase 1: Discovering author subcategories in '{full_cat_title}'..."
        )

        author_cat_titles = [
            cat["title"].split(":", 1)[1]
            async for cat in client.get_subcategories_generator(
                full_cat_title.split(":", 1)[1], self.config.lang
            )
        ]
        logger.info(
                ...
                try:
                    ...
                    if self.tree_logger:
                        self.tree_logger.add_node(
                            author_cat, parent_title, page_title, page_type, reason
                        )
                    ...
                    if page_type == PageType.POEM:
                        try:
                            ...
                            logger.warning(
                                f"Page '{page_title}' looked like a poem but failed parsing: {e}"
                            )
                            self.skipped_counter += 1

                    elif page_type in [
                        PageType.POETIC_COLLECTION,
                        PageType.MULTI_VERSION_HUB,
                    ]:
                        logger.info(
                            f"Page '{page_title}' is a {page_type.name}. Extracting and enqueuing sub-pages."
                        )
                    ...
                    logger.error(
                        f"Error processing page {page_title} (ID: {page_id}): {e}",
                        exc_info=False,
                    )
                    self.skipped_counter += 1
                finally:
